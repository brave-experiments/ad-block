 ...
 *                    currently the only used rules are "keepRuleText",
 *                    which is a boolean flag determine whether to keep
 *                    the original filter rule text, and "publicSuffixRules",
 *                    which is a string describing public suffix rules.
 */
const makeAdBlockClientFromString = (filterRuleData, options) => {
  ...
  return new Promise((resolve) => {
    ...
    if (options && options.publicSuffixRules) {
      client.parsePublicSuffixRules(options.publicSuffixRules)
    }
